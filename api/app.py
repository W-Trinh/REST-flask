...
@app.route('/pays/<string:nom_pays>/regions', methods=['POST'])
def post_region(nom_pays):
    """"Ajoute une région a un pays"""
    code = int(request.args.get("code"))
    nom = request.args.get("nom")
    execute_query("insert into regions (code, nom, id_pays) values (?,?, (select id from pays where nom = ?))", (code, nom, nom_pays))
...
def post_ville_for_departement(code_departement):
    ...
    execute_query("insert into villes (code, nom, departement_id) values (?, ?, (select id from departements where code = ?))", (code_ville, nom_ville, code_departement))
